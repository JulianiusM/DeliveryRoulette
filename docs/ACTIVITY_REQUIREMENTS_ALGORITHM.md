...
This document describes how DeliveryRoulette derives required shift counts for each participant based on slot demand, attendance, and configured rules. The logic is implemented in `src/modules/activity/requirements.ts` and reused by the requirements UI, baseline calculator, and recommendation engine.
